...
const createFormDataFromAccommodation = (accommodation: CreateAccommodation) => {
...
> = (set, get) => ({
    ...state,
    getAccommodations: async ({city, guests, startDate, endDate, pageSize, pageNumber}: AccomodationsFilter) => {
    ...
    createAccommodation: async (accommodation: CreateAccommodation) => {
        ...
        try {
            ...
            const res = await axios.post(
                `${BASE_URL}/api/accomodation/create`, body,
                {
                    headers: {
