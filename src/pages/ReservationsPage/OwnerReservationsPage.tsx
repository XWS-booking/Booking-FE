...
import { ReservationWithCancellations } from '../../store/reservation-store/types/reservation-with-cancellation.type';
        ...
        <Table variant='striped' colorScheme='teal'>
          ...
          <Thead>
            <Tr>
              ...
              <Th>Guests</Th>
              <Th>Status</Th>
              <Th textAlign={'center'}>Guests number of cancellations</Th>
            </Tr>
          </Thead>
          <Tbody>
            {ownersReservationsRes.data &&
              ownersReservationsRes.data.map(
                (item: ReservationWithCancellations) => (
                  <Tr key={item.id}>
                    <Td>{item.accommodation.name}</Td>
                    <Td>
                      {item.accommodation.street}{' '}
                      {item.accommodation.streetNumber}{' '}
                      {item.accommodation.city}, {item.accommodation.country}
                    </Td>
                    <Td>
                      {format(
                    ...
                    <Td textAlign={'center'}>{item.numberOfCancellation}</Td>
                    {item.status === 0 ? (
                      <Td>
                        <Flex gap={'4'}>
                          <Button
